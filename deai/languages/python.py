...
PYTHON_NAME_POOLS = {
    ...
    'list': ['items', 'lst', 'arr', 'data', 'stuff', 'things', 'vals', 'rows', 'records'],
    ...
    'func': ['run', 'go', 'do_it', 'process', 'handle', 'exec', 'calc', 'fetch', 'get_data'],
}
...
class _StyleTransformer(ast.NodeTransformer):
    ...
    def _guess_pool(self, old: str) -> list[str]:
        ...
        if any(k in old_l for k in ('name', 'text', 'str', 'msg', 'title', 'content', 'path')):
            return PYTHON_NAME_POOLS['str']
        if any(k in old_l for k in ('result', 'return', 'output', 'response', 'answer')):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        all_args = [a.arg for a in node.args.args + node.args.posonlyargs + node.args.kwonlyargs]
        self.push_scope(all_args)
        if node.args.vararg:
            self.scopes[-1][node.args.vararg.arg] = self._pick_unique(node.args.vararg.arg)
        if node.args.kwarg:
            self.scopes[-1][node.args.kwarg.arg] = self._pick_unique(node.args.kwarg.arg)
        ...
        if not self.style.use_type_hints:
            for a in node.args.args + node.args.posonlyargs + node.args.kwonlyargs + [node.args.vararg, node.args.kwarg]:
                if a is not None:
                    a.annotation = None
            node.returns = None
        ...
        if not node.name.startswith('_') and self.rng.random() < self.style.private_prefix_rate:
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        if (self.rng.random() < self.style.temp_variable_rate and
                len(node.targets) == 1 and isinstance(node.targets[0], ast.Name)):
            tmp_id = self.rng.choice(PYTHON_NAME_POOLS['temp']) + str(self.rng.randint(1, 99))
    ...
    def visit_Expr(self, node: ast.Expr):
        ...
        if (not self.style.use_docstrings and isinstance(node.value, ast.Constant) and
...
def _human_comment(style: StyleProfile, rng) -> str:
    ...
    if rng.random() < style.typo_rate:
        comment = comment.lower().replace('this', 'dis').replace('sure', 'shure').replace('because', 'bc')
    return comment
...
def _add_comments(source: str, style: StyleProfile, rng) -> str:
    ...
    for line in lines:
        ...
        if stripped.startswith('def ') and rng.random() < style.comment_density:
            ...
            result.append(line)
            result.append(' ' * (indent + style.indent_size) + _human_comment(style, rng))
            continue
        result.append(line)
    return '\n'.join(result)
    ...
    for tok in tokens:
        ...
        if t_type == tokenize.STRING and len(t_string) >= 2:
            ...
            if q == '"' and "'" not in inner and rng.random() < 0.4:
                new_tokens.append((t_type, prefix + "'" + inner + "'", start, end, line))
...
def humanize_python(source: str, style: StyleProfile, seed: Optional[int] = None) -> str:
