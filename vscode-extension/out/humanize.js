...
function humanizeCode(source) {
    const config = vscode.workspace.getConfiguration('deai');
    const engine = config.get('engine', 'ast');
    const style = config.get('style', 'random');
    return new Promise((resolve, reject) => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        const cwd = workspaceFolder ? workspaceFolder.uri.fsPath : process.cwd();
        const script = buildPythonScript(source, engine, style, config);
        const python = (0, child_process_1.spawn)('python', ['-c', script], {
            cwd,
...
function buildPythonScript(source, engine, style, config) {
